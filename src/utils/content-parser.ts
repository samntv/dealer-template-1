 ...
 */
export function parseContentSections(entry: CollectionEntry<'solutions'> | CollectionEntry<'indoor-billboards'>): ParsedContent {
  ...
  if (frontmatter.sections && Array.isArray(frontmatter.sections)) {
    const sections = frontmatter.sections
      ...
      .sort((a: ContentSection, b: ContentSection) => (a.order || 0) - (b.order || 0));
 ...
 */
export function parseMultiSectionContent(entry: CollectionEntry<'solutions'> | CollectionEntry<'indoor-billboards'>): ContentSection[] {
  const frontmatter = entry.data as any;
 ...
 */
export function getSectionVariant(sectionType: string): 'hero' | 'overview' | 'features' | 'value-proposition' | 'default' {
 ...
 */
export function sanitizeContentSection(section: ContentSection): ContentSection {
  return {
    ...section,
